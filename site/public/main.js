...
Blocks = (function() {
  ...
  Blocks.prototype.setUI = function() {
    var doneFollower, donePlace,
      _this = this;
    donePlace = new PIXI.Text("confirm block", {
      font: "25px Arial",
      align: "right"
    });
    ...
    donePlace.click = donePlace.touchstart = function() {
      _this.donePlace();
      _this.drawPlacable();
      return _this.drawFollower();
    };
    stage.addChild(donePlace);
    doneFollower = new PIXI.Text("confirm follower", {
      font: "25px Arial",
      align: "right"
    });
    ...
    doneFollower.click = doneFollower.touchstart = function() {
      return _this.donePlaceFollower(_this.callback);
    };
    stage.addChild(doneFollower);
    return this.place(4, 4, this._blocks[27]);
  };
  ...
  Blocks.prototype.makeOneBlock = function(texture, data) {
    ...
    block.value = (_ref = block.castleFlag === 0) != null ? _ref : {
      1: 2
    };
  ...
  Blocks.prototype.place = function(gx, gy, block) {
    ...
    if (this.grid[gx][gy][3] != null) {
      stage.removeChild(this.grid[gx][gy][3]);
    }
    x = (gx + 1) * 100;
    y = (gy + 1) * 100;
    block.position.x = x;
    ...
    this._blocks.pop();
    this.setRotation(this.grid[gx][gy][4]);
    return this.updateEdges(gx, gy);
  };
  ...
  Blocks.prototype.donePlace = function() {
    ...
    return block.click = block.touchstart = null;
  };
  ...
  Blocks.prototype.drawPlacable = function() {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (this.grid[gx][gy][2] === 0 && angels.length > 0) {
        ...
        this.grid[gx][gy][4] = angels;
        _results.push(stage.addChild(placable));
      } else {
  ...
  Blocks.prototype.setRotation = function(angels) {
    ...
    i = 0;
    return this.currentBlock.click = this.currentBlock.touchstart = function(data) {
      var a;
      a = angels[i % angels.length];
      this.rotation = a * Math.PI / 2;
      this.rotationN = a;
      return i++;
    };
  };
  ...
  Blocks.prototype._castleDFS = function(block) {
    ...
    for (i = _i = 0, _len = neighborCastles.length; _i < _len; i = ++_i) {
      ...
      this._score += b.value;
      console.debug(b.followerPlaced);
      console.debug(this._score);
